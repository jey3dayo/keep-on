 ...
 *   上限）。書き込みは起きていないが、クライアントが「まだ空きがある」と判断して
 *   `add` を選んだ結果として起きうる。つまり、クライアントの期間合計（確定値 +
 *   未確定操作列から算出）が実際のサーバー状態より古い可能性の兆候であり、
 *   再同期が必要（他タブ・他端末での書き込みにより stale snapshot のまま
 *   `add`/`clear` の判定がループする行き止まりを防ぐ）。
...
export function HabitCalendarHeatmap({
  ...
  // 書き込みは起きていないと確定していても、クライアントの期間合計（確定値 + 未確定
  // 操作列から算出）自体が stale な可能性を示す兆候だからである。他タブ・他端末で
  // 同じ期間の枠が埋まった場合、この画面の snapshot は古いままなので「まだ空きがある」
  // と誤判定して add を選び続け、created:false → 再同期なし → 次のタップも同じ古い
  // 合計で add を選ぶ、という行き止まりに陥る（チェックイン済みのセルが clear に
  // 到達できず手動リロードするまで削除できない）。rejected は認証・認可・入力検証の
  // 拒否でクライアントの状態モデルとは無関係なため、再同期は不要（PerformResult の
  // JSDoc 参照）。
  //
