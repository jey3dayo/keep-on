...
**本番実測（2026-09-01、デプロイ `ba13ba5` 後、実ブラウザ・ログイン済み・`/dashboard`）**:

```json
{
  "active": "https://keep-on.jey3dayo.net/sw.js",
  "activeState": "activated",
  "waiting": null,
  "installing": null,
  "controller": "https://keep-on.jey3dayo.net/sw.js",
  "caches": ["keepon-ba13ba586ee9"],
  "toast": false
}
```

リロード後も同一（`waiting: null` / `caches` 1件 / トースト非表示）。確認できたのは次の3点。

1. `active` の scriptURL に `?v=` が付かない → 登録 URL がページのビルドに依存しなくなった（ping-pong の根本原因が除去された）
2. キャッシュがデプロイ ID と一致する1件のみ → ファイルスタンプによるビルドスコープ化が機能し、`activate` の掃除が旧キャッシュ（`keepon-v5` および 031 由来の `keepon-<別 sha>`）を回収した
3. `waiting: null` かつトースト非表示 → 報告された「更新しても消えない」症状は解消

なお ping-pong そのものの発生を修正前に直接観測はしていない（回帰報告を受けた時点で Access セッションが
無く、実機確認より先に修正を投入した）。構造の誤りはコード上で確定しており、修正後の上記実測が
期待状態と一致している。
