...
- 2026-09-02（`HEAD = e78bade`）: plan 035（日付境界のユーザー設定・Linear JEY-638）
- 2026-09-03（`HEAD = a55afd2`）: plan 035（**DONE**: PR #197 merge `a55afd2`。本番実測ログは plan 本文。`drizzle/meta` 不在で生成番号がずれる構造問題は plan の Verify に追記）
...
| 034  | ナビゲーション SWR の stale 提供に鮮度上限を設ける（実機の未スタイル描画）        | P1       | S      | 032        | DONE   |
| 035  | 日付が切り替わる時刻をユーザー設定にする（JEY-638）                               | P2       | L      | —          | DONE   |
