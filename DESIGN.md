...
カレンダーの日付セル（7 列グリッド）のように、単独ボタンより密に並ぶ操作面は `icon-button` の 44px（幅・高さとも 44 CSS px）を満たせないことがある（375px 幅ではカレンダーカードの内余白を除くと 44px を割る）。この例外はカレンダーの日付セルに限定するもので、`icon-button` トークン（44px）自体は変更しない。その場合の下限は WCAG 2.5.8 の 24px（幅・高さとも）とし、狭幅ではそのセルを載せるカードの水平余白を縮めて可能な限り 44px へ寄せる。カレンダーでは、横スクロール、カード枠の撤去、疑似要素によるヒット領域の重ね（隣接セルと重なる）は採らない。セルの縦長化も採らないが、これはカレンダーで幅 44px の代替にならないという理由によるもので、密なグリッド全般での縦長化を禁じるものではない。
