...
密に並ぶ操作面（カレンダーの日付セルのような 7 列グリッド）は `icon-button` の 44px を満たせないことがある（375px 幅ではカード内余白を除くと 44px を割る）。その場合の下限は WCAG 2.5.8 の 24px とし、狭幅ではその面を載せるカードの水平余白を縮めて可能な限り 44px へ寄せる。横スクロール、カード枠の撤去、疑似要素によるヒット領域の重ね（隣接セルと重なる）、セルの縦長化（幅の代替にならない）は採らない。この例外は単独ボタンの `icon-button` トークン（44px）を変更するものではない。
