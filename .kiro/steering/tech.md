...
- `src/lib/sentry.ts` に Sentry 送信ラッパーを集約し、共通タグ/コンテキストの付与を統一
- `SENTRY_DSN` 未設定時は Sentry 送信をスキップし、ローカルログにフォールバック
...
- **HTTP ヘッダー防御**: `next.config.ts` の `headers()` で CSP / X-Frame-Options などを一元適用
