...
- **運用ルートの分離**: `/health` などのヘルスチェックと `/debug/*` の検証ページを通常機能と分離配置
...
- `@/` は `src/` を跨ぐ参照（例: `app` → `lib`, `components` → `schemas`）で優先
- 同一機能・同一ディレクトリ内（例: component と story/test、action と helper）は相対参照を許容
- 深い相対パス（`../../` 以上）が必要になる場合は `@/` に切り替える
...
1. `mise run ci`: CI 相当チェック（型 + Biome + test + build）
